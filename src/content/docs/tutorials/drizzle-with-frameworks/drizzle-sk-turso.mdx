...
This tutorial demonstrates how to build `Todo app` using **Drizzle ORM** with **Turso** and **SvelteKit**.

<Prerequisites>  
- You should have an existing SvelteKit project or create a new one using the following command:
  ```bash
  npx sv create # select drizzle, SQLite and Turso
  ```
...
- You should have installed the [Turso Client Package](https://docs.turso.tech/sdk/ts/reference) and the [Turso CLI](https://docs.turso.tech/cli/installation).

  <Npm>@libsql/client</Npm>
...
<Callout type="warning">
  You can either set up your SvelteKit app either with the
  [`sv`-CLI](https://svelte.dev/docs/cli) or scaffold it out yourself. This
  guide will cover the setup with the CLI.

If you set up your project with the CLI, you will have to change some files, like stated in the steps below. If not, just follow along.
...
In your terminal, enter these commands to create a new database and retrieve it's credentials with the CLI:

```bash
turso db auth login # login or sign up with Turso, if you haven't already
turso db create todo-app # create database
```

#### Setup connection string variable

```bash
turso db show todo-app # show database details, including the URL, which you need
turso db tokens create todo-app # create an authorization-token
```

Add the `DATABASE_URL` environment variable to your `.env` or `.env.local` file, which you'll use to connect to the Neon database.

```bash filename=".env"
DATABASE_URL=libsql://todo-app-<username>.turso.io
DATABASE_TOKEN=eyJ...
```

#### Connect Drizzle ORM to your database

Edit the `index.ts` file in your `src/lib/server/db` folder and set up your database configuration:

```tsx copy filename="src/lib/server/db/index.ts"
...
export const db = drizzle({
  ...
  casing: "snake_case", // or set to "camelCase" if you prefer that.
  // other options
});
```
...
```tsx copy filename="src/lib/server/db/schema.ts"
...
Here we define the **`todo`** table with fields **`id`**, **`text`**, and **`completed`**, using data types from Drizzle ORM. You can specify different database field names in the function invocation.
...
export default defineConfig({
  ...
  driver: "turso",
  dialect: "sqlite",
  casing: "snake_case", // don't forget to add this
});
```
